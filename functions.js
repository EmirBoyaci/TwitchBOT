...
const getAllCommands = async (mongoConnection) => {
    try {
        ...
        return (commandArray.length > 0 ? commandArray.sort((a, b) => {
 ...
 * @returns true, false
 */
const addCommand = async (mongoConnection, commandName, description, userLevel = "everyone") => {
    try {
        const collection = mongoConnection.collection('commands');
...
const editCommand = async (mongoConnection, commandName, description) => {
    try {
        ...
        const isExists = await findCommand(mongoConnection, commandName) !== null;
        if (isExists && commandName.charAt(0) === "!") {
            await collection.updateOne({commandName: commandName}, {
                $set: {
                    description: description,
                },
            });
...
const deleteCommand = async (mongoConnection, commandName) => {
    try {
        ...
        const isExists = await findCommand(mongoConnection, commandName) !== null;
        if (isExists) {
            await collection.deleteOne({commandName: commandName}, {
                commandName: commandName,
            });
            return true;
        } else {
...
const checkGreeting = async (mongoConnection, username) => {
    try {
        const collection = mongoConnection.collection(`greetings-${moment().tz("Europe/Istanbul").format("DDMMYYYY")}`);
    ...
    try {
        ...
        for (const database of dbList.databases) {
            if (database.name !== 'admin' && database.name !== 'local') {
                await mongoClient.db(database.name).dropCollection(`greetings-${moment().tz("Europe/Istanbul").subtract(1, 'days').format("DDMMYYYY")}`, (err, isDeleted) => {
                    if (isDeleted) console.log(`[MONGO-LOG] ${database.name}'s greetings-${moment().tz("Europe/Istanbul").subtract(1, 'days').format("DDMMYYYY")} collection is deleted.`);
                });
            }
        }
    } catch (err) {
...
const getBtc = async () => {
    try {
        const result = await coinMarketClient.getQuotes({id: [1], convert: "TRY"});
...
const setSpotifyAuthorizationCode = async (mongoConnection, authorizationCode) => {
    try {
        ...
        if (!spotifyCredentials) {
            ...
            await collection.updateOne({settingsName: "spotify"}, {
                $set: {
                    spotifyAuthorizationCode: authorizationCode
                }
            });
    ...
    try {
        ...
        if (!spotifyCredentials.spotifyAccessToken) {
            await axios.post('https://accounts.spotify.com/api/token',
                ...
                await collection.findOneAndUpdate({settingsName: "spotify"}, {
                    $set: {
...
const getCurrentPlayingSpotifyTrack = async (mongoConnection) => {
    try {
        const spotifyCredentials = await getSpotifyCredentials(mongoConnection);
        if (spotifyCredentials.spotifyAccessToken) {
            const response = await axios.get('https://api.spotify.com/v1/me/player/currently-playing?market=TR', {
                headers: {
        ...
        for (const database of dbList.databases) {
            if (database.name !== 'admin' && database.name !== 'local') {
                ...
                if (await getSpotifyCredentials(mongoConnection)) {
                    ...
                    if (result) {
                        console.log(`[SPOTIFY-TOKEN-LOG] ${database.name}'s spotify access token refreshed.`);
                    } else {
                        console.error(`[SPOTIFY-TOKEN-LOG] ${database.name}'s spotify access token NOT refreshed.`);
                    }
                } else {
                    console.log(`[SPOTIFY-TOKEN-LOG] ${database.name}'s spotify settings not found.`);
                }
            }
        }
    } catch (err) {
            ...
            if (response) {
                if (response.status === 200) {
                    if (response.data.context) {
                        return response.data.context.external_urls.spotify.match(/playlist/g) !== null ? response.data.context.external_urls.spotify : 404;
                    } else {
                        return 404;
                    }
                } else {
