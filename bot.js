...
twitchClient.on('chat', async (channel, user, message, self) => {
    ...
    const isMod = user.mod || user['user-type'] === 'mod' || displayName === "autoMagic";
    const isBroadcaster = channelName === user.username || displayName === "autoMagic";
    ...
    if (msg.charAt(0) === '!') {
        if (isModUp && arrayMsg[0] === "!komutekle") {
            if (!arrayMsg[1] || !arrayMsg[2]) {
                ...
                return twitchClient.say(channel, `@${displayName}, bot komutlarını tekrar ekleyemezsiniz!`);
            }
            ...
            await addLog(mongoConnection, displayName, "added", arrayMsg[1], getUserLevel(user));
            return twitchClient.say(channel, commandAdded ? `@${displayName}, ${arrayMsg[1]} komutu başarıyla eklendi.` : `@${displayName}, ${arrayMsg[1]} komutu zaten bulunuyor veya hatalı kullanım.`);
        } else if (isModUp && arrayMsg[0] === "!komutdüzenle") {
            if (!arrayMsg[1] || !arrayMsg[2]) {
                return twitchClient.say(channel, `@${displayName}, hatalı kullanım.`);
            } else if (await isCoreCommand(mongoConnection, arrayMsg[1])) {
                return twitchClient.say(channel, `@${displayName}, bot komutlarını düzenleyemezsiniz!`);
            }
            const descriptionPosition = arrayMsg[0].length + arrayMsg[1].length + 2;
            const description = msg.slice(descriptionPosition);
            const commandEdited = await editCommand(mongoConnection, arrayMsg[1], description);
            await addLog(mongoConnection, displayName, "edited", arrayMsg[1], getUserLevel(user));
            return twitchClient.say(channel, commandEdited ? `@${displayName}, ${arrayMsg[1]} komutu başarıyla düzenlendi.` : `@${displayName}, ${arrayMsg[1]} komutu bulunmuyor veya hatalı kullanım.`);
        } else if (isModUp && arrayMsg[0] === "!komutsil") {
            if (!arrayMsg[1]) {
                return twitchClient.say(channel, `@${displayName}, hatalı kullanım.`);
            } else if (await isCoreCommand(mongoConnection, arrayMsg[1])) {
                return twitchClient.say(channel, `@${displayName}, bot komutlarını silemezsiniz!`);
            }
            const commandDeleted = await deleteCommand(mongoConnection, arrayMsg[1]);
            await addLog(mongoConnection, displayName, "deleted", arrayMsg[1], getUserLevel(user));
            return twitchClient.say(channel, commandDeleted ? `@${displayName}, ${arrayMsg[1]} komutu başarıyla silindi.` : `@${displayName}, ${arrayMsg[1]} komutu zaten bulunmuyor.`);
        } else if (isBroadcaster && arrayMsg[0] === "!tümkomutlarısil") {
            const commandDeleted = await deleteAllCommands(mongoConnection);
            await addLog(mongoConnection, displayName, "executed", arrayMsg[0], getUserLevel(user));
            return twitchClient.say(channel, commandDeleted ? `@${displayName}, tüm komutlar başarıyla silindi.` : `@${displayName}, hata lütfen daha sonra tekrar deneyin.`);
        } else if (isModUp && arrayMsg[0] === "!botkomutları") {
            const coreCommands = await getCoreCommands(mongoConnection);
            await addLog(mongoConnection, displayName, "executed", arrayMsg[0], getUserLevel(user));
            return twitchClient.say(channel, coreCommands.length > 0 ? `@${displayName}, kullanılabilir bot komutları: ${coreCommands}` : `@${displayName}, hata lütfen daha sonra tekrar deneyin.`);
        } else if (arrayMsg[0] === "!komutlar") {
            const allCommands = await getAllCommands(mongoConnection);
            await addLog(mongoConnection, displayName, "executed", arrayMsg[0], getUserLevel(user));
            return twitchClient.say(channel, allCommands.length > 0 ? `@${displayName}, kullanılabilir komutlar: ${allCommands}` : `@${displayName}, kullanılabilir komut yok.`);
        }
        const command = await findCommand(mongoConnection, msg);
        if (command) {
            if (command.commandName === "!btc") {
                const btc = await getBtc();
                await addLog(mongoConnection, displayName, "executed", command.commandName, getUserLevel(user));
                return twitchClient.say(channel, `@${displayName}, 1 BTC = ${btc.toLocaleString('tr-tr', {
                    style: 'currency',
                    currency: 'TRY'
                })}`);
            } else if (command.commandName === "!şarkı") {
                const currentPlayingSong = await getCurrentPlayingSpotifyTrack(mongoConnection);
                await addLog(mongoConnection, displayName, "executed", arrayMsg[0], getUserLevel(user));
                if (currentPlayingSong === 401) {
                    return twitchClient.say(channel, `@${displayName}, spotify tokenı tanımsız. Kanal sahibi fısıltı ile !spotifytoken <token> spotify tokenı belirtmelidir.`);
                } else if (currentPlayingSong === 204) {
                    return twitchClient.say(channel, `@${displayName}, şu anda çalan şarkı yok.`);
                } else {
                    return twitchClient.say(channel, `@${displayName}, şu anda çalan şarkı: ${currentPlayingSong}`);
                }
            } else if (command.commandName === "!playlist") {
                const currentPlayingPlaylist = await getCurrentPlayingSpotifyPlaylist(mongoConnection);
                await addLog(mongoConnection, displayName, "executed", arrayMsg[0], getUserLevel(user));
                if (currentPlayingPlaylist === 401) {
                    return twitchClient.say(channel, `@${displayName}, spotify tokenı tanımsız. Kanal sahibi fısıltı ile !spotifytoken <token> spotify tokenı belirtmelidir.`);
                } else if (currentPlayingPlaylist === 404) {
                    return twitchClient.say(channel, `@${displayName}, şu anda çalan şarkı playlistten çalmıyor.`);
                } else if (currentPlayingPlaylist === 204) {
                    return twitchClient.say(channel, `@${displayName}, şu anda çalan şarkı yok.`);
                } else {
                    return twitchClient.say(channel, `@${displayName}, şu anda çalan playlist: ${currentPlayingPlaylist}`);
                }
            }
    ...
    } else if ((arrayGreetings.includes("selam") || arrayGreetings.includes("selamlar")) && !checkGreetings) {
...
twitchClient.on('whisper', async (from, user, message, self) => {
    ...
    if (arrayMsg[0] === "!spotifytoken") {
        ...
        return twitchClient.say(userName, `@${displayName}, spotify tokenı başarıyla eklendi.`);
    }
});